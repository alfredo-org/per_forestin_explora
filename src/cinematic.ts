...
export function cinematicPipeline(scene:B.Scene,camera:B.FreeCamera,software:boolean,mobile=false){
 ...
 if(pipeline){pipeline.fxaaEnabled=true;pipeline.bloomThreshold=.9;pipeline.bloomWeight=.1;pipeline.bloomKernel=32;pipeline.bloomScale=.35;pipeline.samples=1;pipeline.depthOfFieldEnabled=false;}
 let quality:'balanced'|'cinematic'=mobile?'balanced':'cinematic',adaptive=1,elapsed=0,total=0,samples=0;
 const preferenceKey=mobile?'forestin.quality.mobile.v2':'forestin.quality';
 try{const saved=localStorage.getItem(preferenceKey);if(saved==='balanced'||saved==='cinematic')quality=saved;}catch{}
 function apply(){if(software)return;if(pipeline)pipeline.bloomEnabled=quality==='cinematic';const canvas=scene.getEngine().getRenderingCanvas();const density=renderDensity(canvas?.clientWidth||1,canvas?.clientHeight||1,devicePixelRatio,mobile,quality,adaptive);scene.getEngine().setHardwareScalingLevel(1/density);}
 apply();return {mobile,resize:apply,sample(ms:number){if(software||ms>200||ms<1)return;elapsed+=ms;total+=ms;samples++;if(elapsed<2500)return;const next=adaptiveStep(adaptive,total/samples,mobile);elapsed=total=samples=0;if(next!==adaptive){adaptive=next;apply();}},toggle(){quality=quality==='cinematic'?'balanced':'cinematic';adaptive=1;apply();try{localStorage.setItem(preferenceKey,quality);}catch{}},label(){return software?'Modo compatible':mobile?(quality==='cinematic'?'Móvil · detalle':'Móvil · fluida'):quality==='cinematic'?'Cinematográfica':'Equilibrada';}};
}
