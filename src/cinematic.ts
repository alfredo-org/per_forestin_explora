...
export function cinematicPipeline(scene:B.Scene,camera:B.FreeCamera,software:boolean){
 ...
 apply();return {toggle(){quality=quality==='cinematic'?'balanced':'cinematic';apply();try{localStorage.setItem('forestin.quality',quality);}catch{}},label(){return software?'Modo compatible':quality==='cinematic'?'Cinematográfica':'Equilibrada';}};
}
