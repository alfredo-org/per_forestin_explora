export type RenderPreference='balanced'|'cinematic';
...
export function renderDensity(width:number,height:number,dpr:number,mobile:boolean,preference:RenderPreference,adaptive=1){
 const desired=mobile?(preference==='cinematic'?1.15:.9):(preference==='cinematic'?1.5:1);
 const budget=mobile?850000:2400000;
 return Math.max(.45,Math.min(Math.max(1,dpr),desired*adaptive,Math.sqrt(budget/Math.max(1,width*height))));
}
