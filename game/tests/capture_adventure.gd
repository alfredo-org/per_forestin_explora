...
func run()->void:
	...
	scene.player.camera.top_level=true
	scene.player.camera.global_position=animal.global_position+Vector3(3,1.8,4.5)
	scene.player.camera.look_at(animal.global_position+Vector3.UP*1.2)
	await capture("guanaco.png")
	var report:={"renderer":"OpenGL software CI", "resolution":"1280x720", "target_gpu_tested":false, "visible_objects":Performance.get_monitor(Performance.RENDER_TOTAL_OBJECTS_IN_FRAME), "draw_calls":Performance.get_monitor(Performance.RENDER_TOTAL_DRAW_CALLS_IN_FRAME), "primitives":Performance.get_monitor(Performance.RENDER_TOTAL_PRIMITIVES_IN_FRAME), "nodes":get_node_count(), "target_fps":"not measured"}
