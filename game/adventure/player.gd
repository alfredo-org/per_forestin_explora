...
const Motion=preload("res://adventure/character_motion.gd")
...
func _ready() -> void:
	...
	add_child(collider)
	visual=Shapes.forestin();add_child(visual);animator=Motion.new(visual)
...
func reset_to(pos:Vector3)->void:
	global_position=pos;velocity=Vector3.ZERO;coyote=0;jump_buffer=0;was_grounded=false
