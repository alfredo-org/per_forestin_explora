...
static func forestin() -> Node3D:
	...
	for side in [-1,1]:
		...
		capsule(leg,Vector3(0,-.16,0),.12,.34,boots)
		var knee:=pivot(leg,Vector3(0,-.32,0),"Knee")
		capsule(knee,Vector3(0,-.16,0),.105,.34,boots)
		var ankle:=pivot(knee,Vector3(0,-.32,0),"Ankle")
		ellipsoid(ankle,Vector3(0,0,-.055),Vector3(.26,.23,.43),boots)
		var arm := pivot(root,Vector3(side*0.32,1.19,0),"ArmL" if side<0 else "ArmR")
		capsule(arm,Vector3(side*.025,-.10,0),.105,.25,khaki)
		var elbow:=pivot(arm,Vector3(side*.035,-.22,0),"Elbow")
		capsule(elbow,Vector3(0,-.085,0),.083,.20,khaki)
		ellipsoid(elbow,Vector3(side*.02,-.20,-.025),Vector3(.18,.22,.17),cream)
	...
	for side in [-1,1]:
		...
		var eye_pivot:=pivot(head,Vector3(side*.18,.075,-.25),"EyeL" if side<0 else "EyeR")
		ellipsoid(eye_pivot,Vector3.ZERO,Vector3(.085,.10,.06),eye)
		ellipsoid(eye_pivot,Vector3(-side*.014,.020,-.029),Vector3(.022,.025,.01),cream)
		for j in range(3):
	...
	for side in [-1,1]:
		var leg:Node3D=root.get_node(("LegL" if side<0 else "LegR")+"/Knee/Ankle")
		box(leg,Vector3(0,-.095,-.05),Vector3(.25,.045,.37),boots)
		for j in range(3):box(leg,Vector3(0,.07,-.14+j*.045),Vector3(.16,.015,.016),khaki)
